...
public class Person {
    ...
    private void setBirthID() {
        ...
        int lastTwo = 0;
        String tmp = (getBirthDate().getYear() % 100 < 10 ? "0" : "") + getBirthDate().getYear() % 100 + (getBirthDate().getMonth() < 10 ? "0" : "") + getBirthDate().getMonth() + (getBirthDate().getDay() < 10 ? "0" : "") + getBirthDate().getDay() + (ran < 10 ? "0" : "") + ran;
        while(Long.parseLong( tmp + (lastTwo < 10 ? "0" : "") + lastTwo) % 11 != 0) lastTwo++;
    ...
    public String toString() {
        return getName() + "\nSex: " + (getSex() == 'M' ? "Male" : "Female") + "\n" + getBirthDate().toString() + " (" + getBirthID() + ")" + "\nAge: " + getAge() + "\n" + getAddress().toString();
    }
